function safeLoadLua(file)
    ok, chunk = pcall(love.filesystem.load, file)
    if not ok then
        -- TODO Error report - failed to load settings file
        return false
    else
        ok, result = pcall(chunk)
        if not ok then
            -- TODO Error report - error in lua code
            return false
        else
            return result
        end
...
-- Credit: https://love2d.org/wiki/Additional_math
